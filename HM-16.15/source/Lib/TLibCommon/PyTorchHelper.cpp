...
void PyTorchHelper::PredictFractionalBlocks(const cv::Mat& img, Pel* m_PicFilterdByCNN)
{
    int img_size = img.rows * img.cols;
    ...
    // 예측
    auto test = MyNet.forward(input_tensors);
    torch::Tensor netOutput = MyNet.forward(input_tensors).toTensor();
    netOutput = netOutput.permute({ 0, 2, 3, 1 });
    // std::cout << netOutput.sizes() << std::endl;
    ...
    // 결과값 크기
    size_t output_size = output_tensor.numel();
    std::memcpy(m_PicFilterdByCNN, output_tensor.data_ptr(), sizeof(float) * output_size);
}
